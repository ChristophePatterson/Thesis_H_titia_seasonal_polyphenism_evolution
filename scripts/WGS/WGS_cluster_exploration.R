...
# Hydro basin 5
WGS_hydro5 <- read.table("data/SNPs/SNAPP/Hetaerina_titia_ddRAD_titia_dg-SNAPP-hydro_5_max_cov.txt", header = T)
# Get hydrobasin name
WGS_hydro5$hydro5 <- substr(WGS_hydro5$species, 1, 10)
#Hydrobasin4
## Get samples that were included WGS phylogeny 
WGS_hydro4 <- read.table("data/SNPs/SNAPP/Hetaerina_titia_ddRAD_titia_dg-SNAPP-hydro_4_max_cov.txt", header = T)
# Get hydrobasin name
...
clusters[[8]] <- c(7050050640)
clusters[[9]] <- c(7050054660, 7050054670)
clusters[[10]] <- c(7050043860, 7050043870, 7050045480)
clusters[[11]]  <- c(7050045900)
clusters[[12]]  <- c(7050041410, 7050453940, 7050494360)
clusters[[13]]  <- c(7050609780)
clusters[[14]]  <- c(7050511950, 7050531210)
